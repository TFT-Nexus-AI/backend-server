...
public class RiotWebClientFactory {
	...
	public RiotWebClientFactory(WebClient.Builder webClientBuilder, RiotApiProperties properties) {
		Function<String, WebClient> createClient = (baseUrl) -> webClientBuilder.clone()
			.baseUrl(baseUrl)
			.defaultHeader("X-Riot-Token", properties.key())
			.codecs(configurer -> configurer.defaultCodecs().maxInMemorySize(16 * 1024 * 1024))
			.build();

		this.accountClients = properties.baseUrl()
			.account()
			.entrySet()
			.stream()
			.collect(Collectors.toMap(Map.Entry::getKey, e -> createClient.apply(e.getValue())));

		this.summonerClients = properties.baseUrl()
			.summoner()
			.entrySet()
			.stream()
			.collect(Collectors.toMap(Map.Entry::getKey, e -> createClient.apply(e.getValue())));

		this.matchClients = properties.baseUrl()
			.tftMatch()
			.entrySet()
			.stream()
			.collect(Collectors.toMap(Map.Entry::getKey, e -> createClient.apply(e.getValue())));
