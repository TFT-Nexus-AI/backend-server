...
public class RiotWebClient {
	...
	public AccountDto getAccount(String gameName, String tagLine, Region region) {
		...
		return client.get()
			...
			.onStatus(HttpStatusCode::is4xxClientError, clientResponse -> {
				return Mono.error(new UserNotFoundException(gameName, tagLine));
			})
	...
	private Mono<? extends Throwable> handleError(ClientResponse response) {
		return response.bodyToMono(String.class).defaultIfEmpty("Response body is empty").flatMap(body -> {
			String errorMessage = String.format("API Error: %s, Response: %s", response.statusCode(), body);
